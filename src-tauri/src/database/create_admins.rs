...
pub async fn create_administrator(pool: &PgPool) -> Result<(), Error> {
    let admin_username = env::var("ADMIN_USERNAME").unwrap_or(ADMIN_USERNAME.to_string());
    let admin_email = env::var("ADMIN_EMAIL").unwrap_or(ADMIN_EMAIL.to_string());
    let admin_password = env::var("ADMIN_PASSWORD").unwrap_or(ADMIN_PASSWORD.to_string());
    ...
    let existing_admin =
        sqlx::query("SELECT 1 FROM administrators WHERE username = $1 OR email = $2")
            .bind(&admin_username)
            .bind(&admin_email)
            .fetch_optional(pool)
            .await?;
